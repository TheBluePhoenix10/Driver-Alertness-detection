# -*- coding: utf-8 -*-
"""
Created on Sun Mar 3 09:23:55 2019
...
def weather():
    ...
    if not lat:
        ...
        owm = pyowm.OWM('8cf7c77858afb11209f5bf912d72a09c')
...
def GUI():
    ...
    top = pygame.image.load("icons/top.png")

    temp = pygame.image.load("icons/temperature.png")
    winds = pygame.image.load("icons/windspeed.png")
    humid = pygame.image.load("icons/humid.png")
    # speak_thr.start()
    ...
    while loop:
        ...
        if 1:
           for color, result in zip(colors, results):
               ...
               rec=['traffic light','car','truck','bus','stop sign', 'person', '', '', '']
               ...
               if(a!=None):
                 if(a[0]!=None):
                   ...
                   txt2 = '{} : Dist: {:.01f}cm'.format(a[0], dist/10)
                   frame = cv2.putText(
               ...
               if(confidence*100>0 and (label in rec)):
                   ...
                   text = '{}: Dist: {:.0f}cm'.format(label, confidence * 100, dist/10)
                   frame = cv2.putText(
       ...
       # print(lang)
        ...
        if closestLabel:
          if(closestLabel in vehicles and closestDist < 17*10):
            if(lang==0):
              playsound("./sounds/veh_hindi.mp3")
            elif(lang==1):
              playsound("./sounds/veh_engl.mp3")
            # elif(lang==2):
            ...
            if(closestLabel=="stop sign"):
              if(lang==0):
                playsound("./sounds/stop_hindi.mp3")
              elif(lang==1):
                playsound("./sounds/stop_eng.mp3")
              # elif(lang==2):
              #   playsound("./sounds/stop_mh.mp3")
              # elif(lang==3):
              #   playsound("./sounds/stop_bn.mp3")
            # if(closestLabel=="person"):
            #   if(lang==0):
